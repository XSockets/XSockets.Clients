...
namespace XSockets
{
    ...
    public partial class Controller : IController
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine(this.XSocketClient.Serializer.SerializeToString(message));
            if (message.MessageType == MessageType.Text)
                return GetDataFrame(FrameType.Text, Encoding.UTF8.GetBytes(this.XSocketClient.Serializer.SerializeToString(message)));//return GetDataFrame(FrameType.Text, Encoding.UTF8.GetBytes(message.ToString()));
            return GetDataFrame(FrameType.Binary, message.ToBytes());
        }

        [Obsolete("SetProperty should be used instead")]
